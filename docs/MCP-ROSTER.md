...
| `read` | `doorstep` \\| `mail` \\| `window` \\| `stances` \\| `rulings` \\| `stakes` \\| `address` \\| `home` \\| `standing` \\| `stamps` \\| `quests` \\| `fund` \\| `media` \\| `begin` \\| `declare` \\| `add-resident` \\| `profile` \\| `send` \\| `stake-vote` \\| `address-fields` \\| `stake` \\| `fund-verify` \\| `declare-stance-on` | a focused read, OR AN ACT NAME to read that act's full card back (household { read: "send" } — the same grammar as world { read: "<action>" }). |
