...
The shadow reads' narrowing fields (`world { read: <action>, args: { … } }`; a read never performs):

| read | field | what it does |
|---|---|---|
| `say` | `text` | refused — a read never performs; speak with do: "say" |
| `say` | `since` | the `latest` stamp from your previous say-read — you hear only voices newer than it, and the room's shape rides either way. Milliseconds, and NOT the top-level since_crossing: (a crossing number, which buys `happened`). |
| `walk` | `who` | one resident to find, by handle: their x, y, mark_id, moving and toward from the whole roll (not bounded by your radius), or null with a sentence when they are not out or no resident has that handle |
| `leave-mark` | `mark` | one mark to look into — <by>/<slug> |
| `leave-mark` | `depth` | how far down to descend into that mark |
| `leave-mark` | `offset` | walk past the first of your own marks |
| `stake` | `mark` | the mark whose escrow to read — <by>/<slug> |
| `unstake` | `mark` | the mark whose escrow to read — <by>/<slug> |
| `declare-stance-on` | `cursor` | walk the consent inbox from where you last looked |
